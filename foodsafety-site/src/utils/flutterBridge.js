...
export function initFlutterBridge() {
  ...
  if (typeof window === 'undefined' || typeof window.flutterObject === 'undefined') {
    addDebugLog('warn', 'Flutter bridge not available - running outside of app');
    return false;
  }
...
function handleFlutterMessage(event) {
  try {
    ...
    addDebugLog('error', 'Error handling flutter message', { error: error.message, stack: error.stack });
  }
}
...
export function callFlutterMethod(methodName, data = null) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // 如果發送消息失敗，立即reject
      addDebugLog('error', `Failed to send Flutter message: ${methodName}`, { error: error.message });
      reject(error);
    }
  });
}
...
export async function getUserInfo() {
  try {
    ...
    addDebugLog('error', 'Error getting user info', { error: error.message });
    return null;
  }
}
...
export async function getCurrentLocation() {
  try {
    ...
    addDebugLog('error', 'Error getting location', { error: error.message });
    return null;
  }
}
